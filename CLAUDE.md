...
- **Made repeated, confidently-wrong claims about another repo's (upstream/dev) state without checking the exact target.** Claimed a whole custom dashboard was this fork's own build — it was upstream's own file, byte-identical since the fork point. Claimed a zoom option "doesn't exist upstream" by checking the wrong (stock, not the actual Boost-dashboard) file. Prepared to hand over a whole-file diff without checking whether the preference keys it referenced existed on the target, which would not have compiled. Ran `git log --author="<own name>|Claude"` to find "our own" work, got an empty result for three large features, and concluded they were unclaimed — the actual git author was the upstream maintainer, and an empty filter result was silently treated as a real answer instead of a signal to check differently. → Before stating anything about another repo/branch/author's state, verify against the *exact* file, commit, or author field directly (`git show <ref>:<path>`, `git log -1 --format=%an <sha>`) — never infer from a same-named sibling, a conceptually similar file, or a filter whose empty result was never sanity-checked against a known-positive case.
